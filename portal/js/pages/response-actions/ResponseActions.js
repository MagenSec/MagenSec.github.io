...
const ACTION_CATALOG = [
    { type: 'Probe', label: 'Probe', icon: 'ti ti-radar-2', tooltip: 'Collect a comprehensive security posture report from the device', enabled: true },
...
function buildResyncOptions(actionType) {
    ...
    if (actionType === 'InventoryScan' || actionType === 'RefreshInventory') {
        return {
            inventory: 'resync',
            compliance: 'none',
            reset: 'none'
...
function isActionEnabledForUser(action, isSiteAdmin) {
    if (!action?.enabled) return false;
    if (action.siteAdminOnly && !isSiteAdmin) return false;
    return true;
}
...
export class ResponseActionsPage extends Component {
    ...
    async queueCommand(useOrgFanout = false) {
        ...
        if (!isActionEnabledForUser(action, this.state.isSiteAdmin)) {
        ...
        try {
            ...
            const nextCheckText = pollHint?.nextCheckAt
                ...
                : '';
            window.toast?.show?.(`${effectiveCommandType} queued for ${targetCount} device(s).${nextCheckText}`, 'success');
    ...
    toggleAction(actionType) {
        ...
        if (!isActionEnabledForUser(action, this.state.isSiteAdmin)) return;
                ...
                <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Probe Report · ${this.state.selectedProbeDeviceName || 'Device'}</h5>
                            <button
                                type="button"
                                class="btn-close"
                ...
                <div class="container-xl">
                    <div class="row g-2 align-items-center">
                        <div class="col">
                            ...
                            <div class="text-muted">Fast command center with action controls and heartbeat-aware tracking. Hard refresh: use Hard Resync.</div>
                        </div>
                        <div class="col-auto">
                                        ...
                                        <div class="row g-4">
                                                ...
                                                <div class="response-panel h-100">
                                                    ...
                                                    <div class="response-grid">
                                                        ${ACTION_CATALOG.map(action => {
                                                            const isEnabledForUser = isActionEnabledForUser(action, isSiteAdmin);
                                                            ...
                                                            return html`
                                                                <div class="response-grid-item">
                                                                    <button
                                                                        ...
                                                                        disabled=${!isEnabledForUser || queueing}
                                                                        ...
                                                                        ${!isEnabledForUser ? html`<div class="response-tape-ribbon">${action.siteAdminOnly ? 'SiteAdmin' : 'Out of service'}</div>` : ''}
                                                                    </button>
                                                                </div>
                                                            `;
                                                        })}
                                            ...
                                            <div class="col-md-4 d-flex flex-column align-items-center justify-content-center">
                                                <div class="response-fire-btn-container w-100 h-100 d-flex flex-column align-items-center justify-content-center position-relative">
                                                    ...
                                                    <button class="response-fire-btn" disabled=${queueing || !isActionEnabledForUser(selectedAction, isSiteAdmin)} onClick=${() => this.queueForSelectedDevices()}>
