...
export class ClientDevicePage extends window.Component {
    ...
    renderCveModal() {
        const { selectedCve, authCtx, manualOrgId } = this.state;
        if (!selectedCve?.cveId) return '';

        return html`
            <${CveDetailsModal}
                cveId=${selectedCve.cveId}
                orgId=${authCtx?.orgId || manualOrgId || ''}
                isOpen=${true}
                onClose=${() => this.closeCveModal()}
            />
        `;
    }
            ...
            <div style="animation: cd-fade-in 0.25s ease-out;">
                <div class="cd-highlights-grid">
                    <div class=${`cd-high-card cd-high-${riskTone}`} style="position:relative;" title="Composite endpoint risk posture score based on vulnerabilities and telemetry hygiene.">
                        ...
                        <div class="cd-high-title">Hygiene Score</div>
                ...
                <div class="cd-highlights-focus-grid">
                    ...
                    >
                        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:6px; gap:8px;">
                            ...
                            <span class="badge bg-danger text-white">Exploited</span>
                        </div>
                    ...
                    <div class="cd-chart-card">
                        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:6px; gap:8px;">
                            ...
                            <span class="badge bg-success text-white">Avg exploit likelihood ${insights.epssAvg}%</span>
                        </div>
                        <div class="cd-kpi-meta" style="margin-bottom:6px;">High exploit risk (≥0.70): ${insights.epssHigh}</div>
                    ...
                    <div class="cd-grid-row" style="grid-template-columns: minmax(0, 1fr) minmax(0, 3fr) minmax(0, 1fr); min-width: 760px; cursor:pointer;" onClick=${() => this.openCveModal(cve)}>
                        <div>
                            ...
                            <div style="margin-top:4px;">
                                ...
                                ${cve.hasKnownExploit ? html`<span class="cd-tag cd-tag-kev" style="margin-left:4px;">Exploited</span>` : ''}
