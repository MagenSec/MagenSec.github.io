...
export class DashboardPage extends Component {
    ...
    componentWillUnmount() {
        ...
        this.destroyCharts();
        destroyChart(this.deviceSparklineChart);
        destroyChart(this.scoreSparklineChart);
    }
                ...
                <div class="card-body">
                    ...
                    <div class="text-muted small mt-3">
                        ...
                        <div class="d-flex gap-2 flex-wrap">
                            <span class="badge bg-info-lt text-info">Total mitigated: ${mitigated.total}</span>
                            <span class="badge bg-info-lt text-info">Critical: ${mitigated.critical}</span>
                            <span class="badge bg-info-lt text-info">High: ${mitigated.high}</span>
                            <span class="badge bg-info-lt text-info">Medium: ${mitigated.medium}</span>
                            <span class="badge bg-info-lt text-info">Low: ${mitigated.low}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    destroyCharts() {
        const charts = [
            'radarChart', 'threatChart', 'complianceChart', 'coverageChart',
            'savingsChart', 'deviceRiskChart', 'cveAgingChart',
            'threatDonutChart', 'complianceDonutChart'
        ];
        for (const name of charts) {
            destroyChart(this[name]);
            this[name] = null;
        }
    }
    ...
    renderSecurityOfficerBanner() {
        ...
        const { signalLine, deliveryLine, tooltips } = buildOfficerNoteStatusCopy({
        ...
        const bgColor     = isUrgent ? 'var(--tblr-danger-lt)' : 'var(--tblr-warning-lt)';
        const borderColor = isUrgent ? 'var(--tblr-danger)' : 'var(--tblr-warning)';
        const accentColor = isUrgent ? 'var(--tblr-danger)' : 'var(--tblr-warning)';
        const titleColor  = isUrgent ? 'var(--tblr-danger)' : 'var(--tblr-warning)';
                ...
                ">
                    ...
                    <div class="d-flex align-items-start gap-3" style="padding: 12px 16px 14px;">
                        ...
                        <div style="flex: 1; min-width: 0;">
                            ...
                            ${topActionText ? html`
                                <div style="font-size: 12px; color: var(--tblr-body-color); margin-bottom: 4px; overflow-wrap: anywhere; word-break: break-word;">
                                    <strong>Priority:</strong> ${topActionText}
                                </div>
                            ` : ''}
                            <div style="font-size: 11px; color: var(--tblr-secondary);">
                                Score ${score}/100 · <span title="${tooltips?.signal || ''}">${signalLine}</span>
                                <br /><span title="${tooltips?.source || ''}">${deliveryLine}</span>
    ...
    renderHero(riskScore, riskColor) {
        const { securityGrade, lastScan, nextScan, coverage, generatedAt, threatSummary, actions } = this.state;
        const coverageTotal = coverage.total || 0;
        const healthyPct = this.getCoveragePercents().healthyPct;
        const coverageLabel = coverageTotal ? `${coverage.healthy}/${coverageTotal} healthy signals` : 'No devices yet';
        const criticalHigh = (threatSummary?.critical || 0) + (threatSummary?.high || 0);
        const openQueue = Array.isArray(actions) ? actions.length : 0;
        const monitoringGapCount = (coverage.stale || 0) + (coverage.offline || 0);
            ...
            <div class="card mb-3" style="background: linear-gradient(135deg, #0f172a 0%, #1657a8 100%); border: none; box-shadow: 0 4px 16px rgba(15,23,42,0.3) !important;">
                <div class="card-body text-white">
                    <div class="row g-4 align-items-center">
                        <div class="col-lg-7">
                            <div class="text-uppercase text-white-50 fw-semibold mb-2 small">Protect command overview</div>
                            <div class="d-flex align-items-center mb-3">
                                ...
                                <div>
                                    ...
                                    <div class="text-white-70"><strong>${criticalHigh}</strong> critical/high exposures · <strong>${openQueue}</strong> actions in queue</div>
                                </div>
                            </div>
                            <div class="row g-2 text-white-80 small mb-3">
                                <div class="col-sm-4"><strong>Signal updated:</strong> ${this.formatTimestamp(lastScan)}</div>
                                <div class="col-sm-4"><strong>Monitoring gaps:</strong> ${monitoringGapCount}</div>
                                <div class="col-sm-4"><strong>Coverage:</strong> ${coverageLabel}</div>
                            </div>
                            <div class="btn-list mt-3">
                                <a href="#!/alerts" class="btn btn-white">
                                ...
                                <a href="#!/posture" class="btn btn-outline-light">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-sm me-1" width="16" height="16" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                        ...
                        <div class="col-lg-5 mt-3 mt-lg-0">
                            <div class="row g-3">
                                <div class="col-6">
                                    <div class="text-white-50 small text-uppercase mb-1">Signal coverage</div>
                                    <div class="h2 mb-0">${healthyPct}%</div>
                                    <div class="text-white-70 small">${coverageLabel}</div>
                                    <div class="progress progress-sm mt-2" style="background: rgba(255,255,255,0.2);">
                                        <div class="progress-bar bg-white" style="width: ${healthyPct}%"></div>
                                    </div>
                                </div>
                                <div class="col-6">
                                    <div class="text-white-50 small text-uppercase mb-1">Next report</div>
                                    <div class="h4 mb-0">${nextScan}</div>
                                    <div class="text-white-50 small mt-2">Updated ${this.formatTimestamp(generatedAt)}</div>
                                </div>
                                <div class="col-6">
                                    <div class="text-white-50 small text-uppercase mb-1">Open queue</div>
                                    <div class="h4 mb-0">${openQueue}</div>
                                    <div class="text-white-70 small">Needs attention now</div>
                                </div>
                                <div class="col-6">
                                    <div class="text-white-50 small text-uppercase mb-1">Critical / high</div>
                                    <div class="h4 mb-0">${criticalHigh}</div>
                                    <div class="text-white-70 small">Exposure pressure today</div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    renderHighlights() {
        const { deviceStats, threatSummary, complianceSummary, coverage, actions } = this.state;
        const atRisk = (deviceStats.blocked || 0) + (deviceStats.disabled || 0);
        const criticalHigh = (threatSummary.critical || 0) + (threatSummary.high || 0);
        const openQueue = Array.isArray(actions) ? actions.length : 0;
        const monitoringGaps = (coverage.stale || 0) + (coverage.offline || 0);
        ...
        return html`
            <div class="row row-cards mt-3">
                <div class="col-sm-6 col-lg-3">
                    <div class="card">
                        <div class="card-body">
                            <div class="text-muted text-uppercase fw-semibold small">Open queue</div>
                            <div class="h2 mb-0">${openQueue}</div>
                            <div class="text-muted small">Recommended actions awaiting triage</div>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6 col-lg-3">
                    <div class="card">
                        <div class="card-body">
                            <div class="text-muted text-uppercase fw-semibold small">Critical / high exposure</div>
                            <div class="h2 mb-0">${criticalHigh}</div>
                            <div class="text-muted small">Threats that should drive patching first</div>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6 col-lg-3">
                    <div class="card">
                        <div class="card-body">
                            <div class="text-muted text-uppercase fw-semibold small">Monitoring gaps</div>
                            <div class="h2 mb-0">${monitoringGaps}</div>
                            <div class="text-muted small">${atRisk} blocked/disabled · stale/offline need review</div>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6 col-lg-3">
                                    ...
                                    <div class="flex-fill">
                                        <div class="d-flex align-items-center gap-2 flex-wrap">
                                            ...
                                            ${epssScore !== null && epssScore > 0.5 ? html`
                                                <span class="badge bg-warning-lt text-warning" title="${Math.round(epssScore * 100)}% likelihood of exploitation">${Math.round(epssScore * 100)}% exploit likelihood</span>
                                            ` : ''}
    ...
    formatTimestamp(value) {
        ...
        return parsed.toLocaleString([], sameDay
