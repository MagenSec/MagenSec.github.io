...
export function buildOfficerNoteStatusCopy({ signalUpdatedText, reportCard }) {
  ...
  const sourceMap = {
    cron:     { label: 'Comms Unit',    hint: 'Scheduled automated report' },
    portal:   { label: 'Field Console', hint: 'Triggered from portal' },
    admin:    { label: 'Command Desk',  hint: 'Admin-initiated action' },
    backfill: { label: 'Records Unit',  hint: 'Historical data backfill' }
  };
  const sourceEntry = sourceMap[sourceRaw.toLowerCase()] || { label: sourceRaw, hint: '' };
  const source = sourceEntry.label;
  ...
  return {
    ...
    tooltips: {
      ...
      report: 'Your daily security summary report',
      source: sourceEntry.hint
    }
  };
}
