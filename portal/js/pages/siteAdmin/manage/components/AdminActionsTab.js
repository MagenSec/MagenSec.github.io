...
export function AdminActionsTab({ orgs = [], onTriggerCron, onResetRemediation, setMainSection, setActiveTab, loadCronStatus }) {
    ...
    const loadTaskConflicts = async () => {
        try {
            const response = await window.api.get('/api/v1/admin/cron/status');
    ...
    const buildFallbackCatalogFromTasks = (tasks) => {
        ...
        return [
            ...
            },
            {
                groupId: 'intel',
                title: 'Intel',
                description: 'Independent cadence jobs for enrichment, remediation action detection, and performance aggregation.',
                jobs: [
                    ...
                    mkJob({ groupId: 'intel', jobId: 'remediation-action', title: 'Remediation Action', description: 'Detect app remediation updates, uninstall actions, and other remediation signals.', taskId: 'AppRemediation Detection', supportsOrg: true, supportsRange: true }),
                    mkJob({ groupId: 'intel', jobId: 'perf-aggregation', title: 'Perf Aggregation', description: 'Aggregate performance telemetry and purge raw data on the configured cadence.', taskId: 'Perf Aggregation', supportsOrg: true, supportsRange: true })
                ]
            },
            {
                groupId: 'maintenance',
                title: 'Maintenance',
                description: 'Lifecycle and cleanup tasks for telemetry, artifacts, and generated storage.',
                jobs: [
                    mkJob({ groupId: 'maintenance', jobId: 'telemetry-cleanup', title: 'Telemetry Cleanup', description: 'On-demand retention cleanup for raw telemetry tables.', taskId: 'Telemetry Cleanup', supportsOrg: true, supportsRange: true, supportsRetention: true }),
                    mkJob({ groupId: 'maintenance', jobId: 'command-artifact-cleanup', title: 'Command Artefact Cleanup', description: 'Daily cleanup of retained command artifacts as backup enforcement for lifecycle policy.', taskId: 'Command Artifact Cleanup', supportsOrg: true, supportsRange: true, supportsRetention: true }),
                    mkJob({ groupId: 'maintenance', jobId: 'blob-cache-lifecycle', title: 'Blob-Cache Lifecycle', description: 'Daily lifecycle cleanup of blob-backed snapshots and aged cache rows.', taskId: 'Blob Lifecycle Cleanup', supportsOrg: true, supportsRange: true, supportsRetention: true })
                ]
            },
            {
                groupId: 'custom',
                title: 'Custom',
                description: 'Operator-only jobs reserved for migrations and future custom workflows.',
                jobs: [
                    mkJob({ groupId: 'custom', jobId: 'telemetry-migration', title: 'Telemetry Migration', description: 'Custom on-demand telemetry migration hook.', taskId: 'Telemetry Migration' })
                ]
            }
        ];
    };
    ...
    const handleTriggerCron = async (group, job) => {
        ...
        const modeLabel = job.supportsMode && params.mode ? ` [${params.mode}]` : '';
        ...
        try {
            const request = {
                ...
                jobId: job.jobId,
                taskId: job.taskId,
                mode: job.supportsMode ? (params.mode || undefined) : undefined,
                orgId: job.supportsOrg ? (params.orgId || undefined) : undefined,
                startDate: job.supportsRange ? (params.startDate || undefined) : undefined,
                endDate: job.supportsRange ? (params.endDate || undefined) : undefined,
                            ...
                            ${job.supportsRange && html`
                                <div class="col-md-4">
                                    ...
                                    <div class="d-flex gap-2 align-items-center">
                                        <input type="date" class="form-control" value=${params.startDate || ''} onChange=${(e) => updateGroupParams(group.groupId, { startDate: e.target.value })} disabled=${isTriggering} />
                                        <span class="text-muted">–</span>
                                        <input type="date" class="form-control" value=${params.endDate || ''} onChange=${(e) => updateGroupParams(group.groupId, { endDate: e.target.value })} disabled=${isTriggering} />
                                    </div>
                                </div>
                            `}
