 ...
 * Legacy AI report generation and viewing
 */
export class AIPosturePage extends Component {
    ...
    async loadReports() {
        ...
        try {
            // Fetch latest report from ai-analyst reports list
            const list = await api.getAIReports(currentOrg.orgId);
            const reports = list?.reports || list?.data?.reports || list?.data || [];
            const latest = Array.isArray(reports) ? reports[0] : null;

            if (latest?.reportId) {
                const detail = await api.getAIReport(currentOrg.orgId, latest.reportId);
                this.setState({ currentReport: detail?.data || detail, loading: false, error: null });
            } else {
                this.setState({ currentReport: null, loading: false, error: null });
            }
        } catch (err) {
    ...
    async generateReport() {
        ...
        try {
            ...
            if (res.success && res.data) {
                logger.info('[AI Posture] Report generated');
                const reportId = res.data.reportId || res.data.reportID || res.data.id;
                if (reportId) {
                    const detail = await api.getAIReport(currentOrg.orgId, reportId);
                    this.setState({ currentReport: detail.data || detail });
                }
            } else {
    ...
    renderReportContent() {
        ...
        let reportContent = currentReport.report || currentReport.content || '';
        ...
        const riskScore = currentReport.riskScore || 0;
        const completedAt = currentReport.completedAt || currentReport.generatedAt;
