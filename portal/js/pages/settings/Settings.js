...
export function SettingsPage() {
    ...
    const handleAddTeamMember = async () => {
        ...
        const normalizedEmail = teamEmail.trim();
        const payload = {
        ...
        try {
            ...
            if (res.success) {
                ...
                setMembers((prev) => [...prev, { userId: normalizedEmail, userEmail: normalizedEmail, role: teamRole || 'ReadWrite', addedAt: new Date().toISOString(), addedBy: 'you' }]);
                await loadSettings();
            } else {
                showToast(res.message || 'Failed to add team member', 'error');
            }
        } catch (error) {
            logger.error('[Settings] Error adding team member:', error);
    ...
    const handleRemoveTeamMember = async (userId) => {
        ...
        try {
            ...
            logger.error('[Settings] Error removing team member:', error);
    ...
    const handleUpdateTeamRole = async (userId, newRole) => {
        ...
        try {
            ...
            logger.error('[Settings] Error updating team member role:', error);
