...
export class DeviceFilterService {
     ...
     * recent-online (<60m), recent-offline (60m-24h), stale (1-3d), dormant (3-7d), ghosted (>7d), error
    ...
    static getConnectivity(device) {
        ...
        if (!device.lastHeartbeat) return 'error';
        const mins = device.inactiveMinutes;
        if (mins === null || mins === undefined) return 'error';
