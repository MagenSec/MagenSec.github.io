...
export class RiskAnalysisService {
    ...
    static deriveNetworkExposure(telemetryDetail) {
        ...
        const rawExposure = fields.NetworkExposureJson || fields.networkExposureJson || fields.NetworkExposure || fields.networkExposure;
        const ipRaw = fields.IPAddresses || fields.ipAddresses;
        const gatewayRaw = fields.DefaultGateways || fields.defaultGateways;
        const dnsRaw = fields.DnsSuffixes || fields.dnsSuffixes;
        const publicEgressHint = fields.PublicEgressHint || fields.publicEgressHint;
        const proxyType = fields.ProxyType || fields.proxyType;
        ...
        const ips = parseArray(ipRaw);
        const gateways = parseArray(gatewayRaw);
        const dnsSuffixes = parseArray(dnsRaw);
        ...
        const hasAnyData = parsed !== null || (ips && ips.length > 0) || gateways.length > 0 || dnsSuffixes.length > 0;
        if (!hasAnyData) {
        ...
        const gatewayCount = data.GatewayCount ?? data.gatewayCount ?? gateways.length;
        ...
        if (publicIpCount > 0) {
            exposure.label = vpnInterfaces > 0 ? 'Routable via VPN' : 'Public-Routable';
            exposure.badgeClass = vpnInterfaces > 0 ? 'bg-warning-lt text-warning' : 'bg-danger-lt text-danger';
            exposure.reasons.push(`${publicIpCount} public-routable IP${publicIpCount > 1 ? 's' : ''}`);
        ...
        if (gatewayCount > 0) {
            exposure.reasons.push(`${gatewayCount} gateway${gatewayCount > 1 ? 's' : ''}`);
        } else if (hasAnyData) {
            exposure.reasons.push('No gateway configured');
        }
        if (publicEgressHint) {
            exposure.reasons.push(`egress ${publicEgressHint}`);
        }
        if (dnsSuffixes.length > 0) {
            exposure.reasons.push(`DNS: ${dnsSuffixes.slice(0, 2).join(', ')}`);
        }
        if (proxyType && proxyType !== 'None') {
            exposure.reasons.push(`Proxy: ${proxyType}`);
        }
        if (apipaPresent) {
