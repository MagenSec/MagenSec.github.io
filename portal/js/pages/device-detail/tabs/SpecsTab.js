...
export function renderSpecsTab(component) {
    ...
    const networkRisk = component.networkService.analyzeNetworkRisk(ipAddresses, telemetryDetail?.history, fields);
    const gateways = component.networkService.parseStringArray(telemetry?.defaultGateways || telemetry?.DefaultGateways || fields.DefaultGateways || fields.defaultGateways);
    const dnsSuffixes = component.networkService.parseStringArray(telemetry?.dnsSuffixes || telemetry?.DnsSuffixes || fields.DnsSuffixes || fields.dnsSuffixes);
    const publicEgressHint = telemetry?.publicEgressHint || telemetry?.PublicEgressHint || fields.PublicEgressHint || fields.publicEgressHint || 'N/A';
    const proxyType = telemetry?.proxyType || telemetry?.ProxyType || fields.ProxyType || fields.proxyType || 'None';
    const isMetered = telemetry?.isMeteredConnection ?? telemetry?.IsMeteredConnection ?? fields.IsMeteredConnection ?? fields.isMeteredConnection;
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <dl class="row text-sm">
                    ...
                    <dt class="col-sm-4">Network Posture</dt>
                    <dd class="col-sm-8">
                        <span class="badge ${networkRisk.badgeClass}">${networkRisk.label}</span>
                        <div class="text-muted small mt-1">${networkRisk.reason}</div>
                    </dd>

                    <dt class="col-sm-4">Gateway / Egress</dt>
                    <dd class="col-sm-8">
                        <div>${gateways.length > 0 ? gateways.slice(0, 2).join(', ') : 'No gateway reported'}</div>
                        <div class="text-muted small">${publicEgressHint}</div>
                    </dd>

                    <dt class="col-sm-4">Proxy / Metered</dt>
                    <dd class="col-sm-8">${proxyType} / ${isMetered ? 'Yes' : 'No'}</dd>

                    <dt class="col-sm-4">DNS Suffixes</dt>
                    <dd class="col-sm-8">${dnsSuffixes.length > 0 ? dnsSuffixes.slice(0, 3).join(', ') : 'None reported'}</dd>
