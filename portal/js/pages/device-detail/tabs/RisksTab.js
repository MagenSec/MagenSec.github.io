...
export function renderRisksTab(component) {
    ...
    const knownExploitCount = component.state.knownExploits ? filteredCves.filter(c => component.state.knownExploits.has(c.cveId)).length : 0;
    const shouldShowAll = component.state.showAllCves || !!component.state.cveFilterApp || filteredCves.length <= 5;
    const displayedCves = shouldShowAll ? filteredCves : filteredCves.slice(0, 5);
    const hiddenCount = Math.max(0, filteredCves.length - displayedCves.length);
    const vulnerableAppCount = new Set(filteredCves.map(c => component.normalizeAppName(c.appName)).filter(Boolean)).size;
    ...
    return html`
        ...
        <div class="mb-3 d-flex flex-wrap gap-2 align-items-center">
            ...
            ${hiddenCount > 0 ? html`
                <span class="badge bg-info-lt text-info">Showing top 5 first</span>
            ` : ''}
        </div>

        ${filteredCves.length > 0 ? html`
            <div class="card bg-primary-lt mb-3 border-0">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                        <div>
                            <div class="text-muted small">What needs attention now</div>
                            <div class="fw-semibold mb-1">
                                ${knownExploitCount > 0
                                    ? `${knownExploitCount} known exploit${knownExploitCount === 1 ? '' : 's'} should be triaged first`
                                    : criticalCves.length > 0
                                        ? `${criticalCves.length} critical vulnerabilit${criticalCves.length === 1 ? 'y' : 'ies'} need action`
                                        : `${vulnerableAppCount} application${vulnerableAppCount === 1 ? '' : 's'} need patch review`}
                            </div>
                            <div class="text-muted small">
                                ${hiddenCount > 0
                                    ? `This device has ${filteredCves.length} active vulnerabilities. The highest-priority 5 are shown first to keep remediation focused.`
                                    : 'All active vulnerabilities are shown below.'}
                            </div>
                        </div>
                        <div class="d-flex gap-2 flex-wrap">
                            <button class="btn btn-primary" onclick=${() => component.openAdvancedDetails('detailApps')}>
                                Review vulnerable software
                            </button>
                            ${hiddenCount > 0 ? html`
                                <button class="btn btn-outline-primary" onclick=${() => component.setState({ showAllCves: true })}>
                                    Show all ${filteredCves.length}
                                </button>
                            ` : ''}
                        </div>
                    </div>
        ...
        <div class="table-responsive">
            ...
            ${hiddenCount > 0 ? html`
                <div class="d-flex justify-content-center pt-3">
                    <button class="btn btn-outline-primary" onclick=${() => component.setState({ showAllCves: true })}>
                        Show remaining ${hiddenCount} vulnerabilities
                    </button>
                </div>
            ` : ''}
            ${component.state.showAllCves && filteredCves.length > 5 ? html`
                <div class="d-flex justify-content-center pt-3">
                    <button class="btn btn-ghost-secondary" onclick=${() => component.setState({ showAllCves: false })}>
                        Show only top 5 again
                    </button>
                </div>
            ` : ''}
