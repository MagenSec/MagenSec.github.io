...
export function renderPerfTab(component, embedded = false) {
    ...
    const netPercentiles = component.calculatePercentiles(points.map(p => bytesToMegabytes((p.networkBytesSent || 0) + (p.networkBytesReceived || 0))));
                ...
                <div class="card h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div class="fw-bold">Client CPU</div>
                            <div class="d-flex gap-1 flex-wrap">
                                ${percentileBadge('P50', cpuPercentiles?.p50, formatPercent)}
                                ${percentileBadge('P90', cpuPercentiles?.p90, formatPercent)}
                                ${percentileBadge('P95', cpuPercentiles?.p95, formatPercent)}
                            </div>
                        </div>
                        <div ref=${(el) => { component.perfCpuEl = el; }} style="min-height: 220px;"></div>
                        <div class="text-muted small mt-2">Latest: ${formatPercent(latestPoint?.cpuAvg)}</div>
                        <div class="text-muted small">Low values are expected when the client is idle and monitoring in the background.</div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-lg-6">
                <div class="card h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div class="fw-bold">Network Usage</div>
                            <div class="d-flex gap-1 flex-wrap">
                                ${percentileBadge('P50', netPercentiles?.p50, formatMegabytes)}
                                ${percentileBadge('P90', netPercentiles?.p90, formatMegabytes)}
                                ${percentileBadge('P95', netPercentiles?.p95, formatMegabytes)}
                            </div>
                        </div>
                ...
                <div class="card h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <div class="d-flex gap-1 flex-wrap">
                                ${percentileBadge('P50', memPercentiles?.p50, formatPercent)}
                                ${percentileBadge('P90', memPercentiles?.p90, formatPercent)}
                                ${percentileBadge('P95', memPercentiles?.p95, formatPercent)}
                            </div>
                        </div>
                        <div ref=${(el) => { component.perfMemEl = el; }} style="min-height: 220px;"></div>
                        <div class="text-muted small mt-2">Latest: ${formatPercent(latestPoint?.memoryAvg)} (${formatMegabytes(latestPoint?.memoryAvgMb)} used)</div>
                ...
                <div class="card h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <div class="d-flex gap-1 flex-wrap">
                                ${percentileBadge('P50', diskPercentiles?.p50, formatMegabytes)}
                                ${percentileBadge('P90', diskPercentiles?.p90, formatMegabytes)}
                                ${percentileBadge('P95', diskPercentiles?.p95, formatMegabytes)}
                            </div>
                        </div>
                        <div ref=${(el) => { component.perfDiskEl = el; }} style="min-height: 220px;"></div>
                        <div class="text-muted small mt-2">Latest total: ${formatMegabytes(latestPoint?.diskTotalMbAvg)} (App ${formatMegabytes(latestPoint?.diskAppMbAvg)}, Intel ${formatMegabytes(latestPoint?.diskIntelMbAvg)})</div>
                    </div>
                </div>
            </div>
        </div>
