...
import { getInstallerConfig } from '@utils/manifestCache.js';
...
export class DeviceDetailPage extends window.Component {
    ...
    async loadInstallerConfig(forceRefresh = false) {
        try {
            ...
            const fallbackInstallers = config.INSTALLERS || this.state.installers;
            const fallbackVersion = fallbackInstallers?.ENGINE?.VERSION || this.state.latestClientVersion || null;
            this.setState({
                installers: fallbackInstallers,
                latestClientVersion: fallbackVersion,
                manifestError: null
            });
            return fallbackInstallers;
        } catch (error) {
            console.error('[DeviceDetail] Failed to load manifest config, using fallback:', error);
            const fallbackInstallers = config.INSTALLERS || this.state.installers;
            const fallbackVersion = fallbackInstallers?.ENGINE?.VERSION || this.state.latestClientVersion || null;
            this.setState({
                installers: fallbackInstallers,
    ...
    isVersionOutdated(deviceVersion) {
        ...
        const latestVersion = this.getLatestInstallerVersion();
        return latestVersion ? this.compareVersions(deviceVersion, latestVersion) < 0 : false;
    }
    ...
    getClientUpdateState() {
        const current = this.state.device?.ClientVersion
            || this.state.device?.clientVersion
            || this.state.deviceSummary?.clientVersion
            || null;
        const latest = this.getLatestInstallerVersion();
        ...
        if (!current || !latest) {
            return {
                ...
                aheadOfBaseline: false,
                postureLabel: current ? 'Version detected' : 'Client version unavailable',
                postureClass: 'text-muted'
            };
        }

        const versionDelta = this.compareVersions(current, latest);
        const updateAvailable = versionDelta < 0;
        const aheadOfBaseline = versionDelta > 0;
        ...
        return {
            ...
            updateAvailable,
            aheadOfBaseline,
            postureLabel: updateAvailable ? 'Update recommended' : aheadOfBaseline ? 'Ahead of published baseline' : 'Up to date',
            postureClass: updateAvailable ? 'text-warning' : aheadOfBaseline ? 'text-info' : 'text-success'
        };
    }
    ...
    getRecommendedNextStep() {
        ...
        if (knownExploitCount > 0) {
            return {
                label: `Review ${knownExploitCount} known exploit${knownExploitCount === 1 ? '' : 's'}`,
                detail: 'These findings should be triaged first.',
        ...
        if (updateState.updateAvailable) {
            return {
                label: `Update client to v${updateState.latest}`,
                detail: 'A newer agent build is available for this device.',
        ...
        return {
            ...
            detail: 'Keep this device monitored and run checks as scheduled.',
            badgeClass: 'bg-success-lt text-success',
            action: 'monitor'
        };
    }

    openAdvancedDetails(sectionId = null) {
        const wrapper = document.getElementById('advancedDetails');
        if (wrapper) {
        ...
        const target = sectionId ? document.getElementById(sectionId) : wrapper;
        if (target) {
            target.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
    }

    runRecommendedNextStep(action) {
        switch (action) {
            case 'updates':
                this.queueDeviceCommand('CheckUpdates');
                break;
            case 'inventory':
                ...
                break;
            case 'risks':
                this.scrollToCveTable();
                break;
            default:
                this.queueDeviceCommand('TriggerScan');
                break;
        }
    }
    ...
    normalizeSummary(summary) {
        ...
        const vulnerableApps = summary.vulnerableAppCount ?? summary.vulnerableApps ?? summary.appsWithCves ?? summary.appWithVulnCount ?? 0;
        ...
        let baseScore = summary.enrichedRiskScore
            ?? summary.EnrichedRiskScore
            ?? summary.riskScore
            ?? summary.score
        ...
        if (finalScore === 0 && (vulnerableApps > 0 || cveCount > 0 || derivedWeight > 0)) {
            const heuristicScore = (cveCount * 2) + (derivedWeight * 10) + (vulnerableApps * 5);
            finalScore = Math.min(100, Math.max(10, heuristicScore));
        }
    ...
    async loadDeviceData() {
        try { 
            ...
            const summary = payloadSummary || decryptedDevice.summary || decryptedDevice.Summary || this.state.deviceSummary;
            ...
            if (deviceSummary) {
                decryptedDevice.summary = summary;
                decryptedDevice.Summary = summary;
            }

            // Deduplicate CVEs using a stable fingerprint.
            // Some payloads repeat the same CVE for the same application with different/blank row keys,
            // so appRowKey alone is not sufficient.
            const dedupeCves = (items) => Array.from(
                new Map((items || []).map(c => {
                    const key = [
                        String(c.cveId || '').trim().toUpperCase(),
                        this.normalizeAppName(c.appName || ''),
                        String(c.vendor || '').trim().toLowerCase(),
                        String(c.appStatus || '').trim().toLowerCase()
                    ].join('|');
                    return [key, c];
                })).values()
            );
            const uniqueCves = dedupeCves(cveList);
            const uniqueMitigatedCves = dedupeCves(mitigatedCveList);
    ...
    normalizePerfAggregation(raw) {
        ...
        const fallbackTotalRamMb = toNumber(
            latestFields.TotalRAMMB
            ?? latestFields.totalRamMb
            ?? this.state.deviceSummary?.telemetry?.hardware?.totalRamMb,
            0
        );
        ...
        norm.points = rawPoints
            .map((p) => {
                ...
                const memoryAvgMb = toNumber(p.memoryAvgMb ?? p.MemoryAvgMb);
                const memoryAvgRaw = toNumber(p.memoryAvg ?? p.MemoryAvg, null);
                const derivedMemoryAvg = fallbackTotalRamMb > 0 && Number.isFinite(memoryAvgMb) && memoryAvgMb > 0
                    ? (memoryAvgMb / fallbackTotalRamMb) * 100
                    : null;
                return {
                    ...
                    memoryAvg: toNumber(memoryAvgRaw !== null && memoryAvgRaw > 0 ? memoryAvgRaw : derivedMemoryAvg, 0),
                    memoryAvgMb,
    ...
    buildDeviceReportModel() {
        ...
        if (networkRisk?.publicIpPresent) {
            recommendations.push('Reduce network exposure: avoid public-routable IPs where possible; enforce firewalling/VPN.');
        }
        if (networkRisk?.apipaPresent) {
            recommendations.push('Investigate network misconfiguration (APIPA addresses detected).');
        }
        if ((networkRisk?.riskFactors || []).some(f => String(f).includes('No default gateway'))) {
            recommendations.push('Review DHCP/default gateway settings to restore stable routing.');
        }
        if (knownExploitCount > 0) {
        ...
        return {
            ...
            network: {
                label: networkRisk?.label || null,
                reason: networkRisk?.reason || null,
                risk: networkRisk?.risk || 'Normal',
                publicIpPresent: !!networkRisk?.publicIpPresent,
                apipaPresent: !!networkRisk?.apipaPresent,
                gatewayCount: networkRisk?.gatewayCount || 0,
                publicEgressHint: networkRisk?.publicEgressHint || null,
                dnsSuffixes: Array.isArray(networkRisk?.dnsSuffixes) ? networkRisk.dnsSuffixes : [],
                mobile: mobileStatus?.isMobile || false
            },
            inventory: {
...
<body>
                    ...
                    <div class="report-hero mb-4">
                        <div class="row g-3 align-items-center">
                            ...
                            <div class="col-md-4 text-md-end">
                                <div class="kpi-card" style="background: var(--tblr-bg-surface, rgba(255,255,255,0.12)); border: 1px solid var(--tblr-border-color, rgba(255,255,255,0.2));">
                                    <div class="kpi-label text-secondary">Risk Score</div>
                                    <div class="kpi-value">${safeRisk} / 100</div>
                                    <div class="text-muted" style="font-size: 12px;">Higher score = greater urgency</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row row-cards mb-3">
                        ...
                        <div class="col-sm-6 col-lg-3">
                            <div class="kpi-card">
                                ...
                                <div class="text-muted">Actively exploited in the wild</div>
                            </div>
                        </div>
                        <div class="col-sm-6 col-lg-3">
                            <div class="kpi-card">
                                <div class="kpi-label">Exposure</div>
                                <div class="kpi-value">${FormattingUtils.escapeHtml(model?.network?.label || (model?.network?.publicIpPresent ? 'Internet' : 'Private'))}</div>
                                <div class="text-muted">${FormattingUtils.escapeHtml(model?.network?.reason || (model?.network?.apipaPresent ? 'APIPA detected' : 'Network healthy'))}</div>
                            </div>
                        </div>
                        <div class="col-sm-6 col-lg-3">
                            <div class="kpi-card">
                    ...
                    <div class="row row-cards mb-4">
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <ul class="mb-0">
                                        <li>${model?.network?.publicIpPresent ? '<span class="text-danger">Public-routable address observed</span>' : 'Private network detected'}</li>
                                        <li>${model?.network?.apipaPresent ? '<span class="text-danger">APIPA detected (DHCP issue)</span>' : 'No APIPA addresses observed'}</li>
                                        <li>${FormattingUtils.escapeHtml(model?.network?.reason || 'Stable network profile')}</li>
    ...
    render() {
        ...
        const updateState = this.getClientUpdateState();
        const latestClientVersion = updateState.latest;
        const updateAvailable = updateState.updateAvailable;
        const nextStep = this.getRecommendedNextStep();
        const actionBadges = [
            updateAvailable ? {
                color: 'bg-warning-lt',
            ...
            : healthStatus === 'offline' || healthStatus === 'recent-offline'
                ...
                : healthStatus === 'stale'
                    ...
                    : healthStatus === 'dormant'
                        ...
                        : healthStatus === 'ghosted' || healthStatus === 'error'
        ...
        const summarySignalSource = this.state.summaryMeta?.source || (this.state.deviceSummary ? 'fresh' : 'unavailable');
        const summarySignalBadgeClass = summarySignalSource === 'cached'
            ? 'bg-warning-lt text-warning'
            : summarySignalSource === 'fresh'
                ? 'bg-success-lt text-success'
                : 'bg-secondary-lt text-secondary';
        ...
        const sessionWindowText = sessionSummary
            ...
            : 'No recent signal intelligence yet';
                                ...
                                <div class="col">
                                    ...
                                    <div class="page-subtitle mt-2">
                                        <div class="d-flex flex-wrap align-items-center gap-2 text-muted small">
                                            ...
                                            ${primaryIp ? html`<span class="font-monospace">${primaryIp}</span>` : ''}
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="btn-list">
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                ...
                                                <a class="dropdown-item" href="#" onclick=${(e) => { e.preventDefault(); this.scrollToCveTable(); this.setState({ cveFilterSeverity: null }); }}>
...
<!-- Metrics Row -->
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row g-3 align-items-center">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                ${(() => {
                                                    ...
                                                    const encoded = f.UserName || f.Username || f.userName || f.LoggedOnUser || f.CurrentUser
                                                        || device.Telemetry?.Username || device.Telemetry?.username
                                                        || device.telemetry?.Username || device.telemetry?.username
                                                        || null;
                                                    const u = encoded ? PiiDecryption.decryptIfEncrypted(String(encoded)) : null;
                                        ...
                                        <div class="row g-3 align-items-center mt-2">
                                            ${(() => {
                                                ...
                                                return html`
                                                    <div class="col-md-4">
                                                        <div class="text-muted small font-weight-medium">Connectivity</div>
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="text-muted small font-weight-medium">Patching</div>
                                                        <div class="d-flex align-items-center gap-2 flex-wrap">
                                                            ${patch.percent !== null ? html`
                                                                <span class="badge ${patchBadgeClass}">${Math.round(patch.percent)}% patched</span>
                                                                ${patch.pending > 0 ? html`<span class="text-muted">${patch.pending} pending</span>` : html`<span class="text-muted">No pending patches</span>`}
                                                            ` : html`
                                                                <span class="text-muted">Patch visibility unavailable</span>
                                                            `}
                                                        </div>
                                                    </div>
                                                `;
                                        ...
                                        <div class="row g-3 align-items-center">
                                            <div class="col-md-4">
                                                ${(() => {
                                                    ...
                                                    return html`
                                                        <div class="d-flex align-items-center gap-3" style="cursor: pointer;" onclick=${(e) => { e.preventDefault(); this.openAdvancedDetails('detailApps'); this.setState({ searchQuery: '' }); }} title="View applications with CVEs">
                                            ...
                                            <div class="col-md-4">
                                                ${(() => {
                                                    ...
                                                    return html`
                                                        <div class="d-flex align-items-center gap-3" style="cursor: pointer;" onclick=${(e) => { e.preventDefault(); this.scrollToCveTable(); }} title="View CVEs">
                                                    ...
                                                    return html`
                                                        <div>
                                                            ...
                                                            <div class="d-flex align-items-center gap-2 flex-wrap mb-1">
                                                                ${clientVersion
                                                                    ? html`<span class="font-monospace fw-bold">v${clientVersion}</span>`
                                                                    : html`<span class="text-muted">Not reported</span>`}
                                                                ${updateAvailable
                                                                    ? html`<span class="badge bg-warning-lt text-warning">Update available → v${latestClientVersion}</span>`
                                                                    : updateState.aheadOfBaseline
                                                                        ? html`<span class="badge bg-info-lt text-info">Ahead of baseline</span>`
                                                                        : clientVersion
                                                                            ? html`<span class="badge bg-success-lt text-success">Up to date</span>`
                                                                            : ''}
                                                            </div>
                                                            <div class="d-flex align-items-center gap-2 flex-wrap">
                                                            ...
                                                            ${this.state.manifestError ? html`<div class="text-muted small mt-1">${this.state.manifestError}</div>` : ''}
                                                        </div>
                                                    `;
                                                })()}
                                            </div>
                        ...
                        <div class="row row-cards mb-3">
                            <div class="col-md-4">
                                <div class="card card-sm h-100">
                                    <div class="card-body">
                                        ...
                                        <div class="text-muted small mt-1">Actively exploited vulnerabilities on this device</div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <div class="card card-sm h-100">
                                    <div class="card-body">
                                        <div class="text-muted small">Recommended next step</div>
                                        <div class="d-flex align-items-start justify-content-between gap-2">
                                            <div class="fw-semibold lh-sm">${nextStep.label}</div>
                                            <span class="badge ${nextStep.badgeClass}">Now</span>
                                        </div>
                                        <div class="text-muted small mt-1">${nextStep.detail}${recentChangeCount > 0 ? ` ${recentChangeCount} recent change${recentChangeCount === 1 ? '' : 's'} tracked.` : ''}</div>
                                        <button class="btn btn-sm btn-outline-primary mt-2" onclick=${() => this.runRecommendedNextStep(nextStep.action)}>
                                            Open
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                                        ...
                                        ` : html`<span class="badge bg-secondary-lt text-secondary">Awaiting signal intelligence</span>`}
                                    </div>
...
<!-- Device Assessment -->
                            ...
                            <div id="advancedDetails" class="collapse">
                                <div class="card-body p-0">
                                    <div class="accordion" id="deviceDetailsAccordion">
                                        ...
                                        <div class="accordion-item border-0">
                                            <h2 class="accordion-header">
                                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#detailSpecs" aria-expanded="false">
                                                    Specifications
                                                </button>
                                            </h2>
                                            <div id="detailSpecs" class="accordion-collapse collapse" data-bs-parent="#deviceDetailsAccordion">
                                                <div class="accordion-body">
                                                    ${this.renderSpecsTab()}
                                                </div>
                                            </div>
                                        ...
                                        <div class="accordion-item border-0">
                                            <h2 class="accordion-header">
                                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#detailTimeline" aria-expanded="false">
                                                    Timeline
                                                    ${(this.state.timeline?.length || 0) > 0 ? html`<span class="badge bg-secondary-lt text-secondary ms-2">${this.state.timeline.length}</span>` : ''}
                                                </button>
                                            </h2>
                                            <div id="detailTimeline" class="accordion-collapse collapse" data-bs-parent="#deviceDetailsAccordion">
                                                <div class="accordion-body">
                                                    ${this.renderTimelineTab()}
                                                </div>
                                            </div>
    ...
    renderPerfCharts() {
        ...
        const roundMetric = (val) => {
            const n = Number(val);
            return Number.isFinite(n) ? Math.round(n * 100) / 100 : 0;
        };

        const bytesToMegabytes = (val) => {
            const n = Number(val);
        ...
        const formatMegabytes = (val) => {
            const n = Number(val);
            if (!Number.isFinite(n)) return '0 MB';
            if (n === 0) return '0 MB';
            if (Math.abs(n) < 1) return `${n.toFixed(2)} MB`;
            if (Math.abs(n) < 10) return `${n.toFixed(1)} MB`;
        ...
        const buildPercentAxis = (values, title) => {
            ...
            return {
                ...
                forceNiceScale: true,
                labels: { formatter: (value) => formatPercent(value) },
                title: { text: title }
            };
        };
        ...
        const points = validPoints
            .map(p => ({
                ts: p.__ts,
                cpu: clampPercent(p.cpuAvg ?? p.CpuAvg),
                memPct: clampPercent(p.memoryAvg ?? p.MemoryAvg),
                memMb: roundMetric(p.memoryAvgMb ?? p.MemoryAvgMb),
                diskTotal: roundMetric(p.diskTotalMbAvg ?? p.diskAvg ?? p.DiskAvg),
                diskApp: roundMetric(p.diskAppMbAvg ?? p.DiskAppMbAvg),
                diskIntel: roundMetric(p.diskIntelMbAvg ?? p.DiskIntelMbAvg),
                netSentBytes: roundMetric(p.networkBytesSent ?? p.NetworkBytesSent ?? 0),
                netRecvBytes: roundMetric(p.networkBytesReceived ?? p.NetworkBytesReceived ?? 0),
                netUploadMb: bytesToMegabytes(p.networkBytesSent ?? p.NetworkBytesSent ?? 0),
        ...
        const chartConfigs = [
            {
                ...
                colors: ['#206bc4'],
                yaxis: [buildPercentAxis(points.map(p => p.cpu), 'Client CPU (%)')],
                tooltipFormatter: (val) => formatPercent(val),
                annotations: buildAnnotations(this.calculatePercentiles(points.map(p => p.cpu)), (v) => formatPercent(v))
            },
            {
                key: 'mem',
                el: 'perfMemEl',
                ...
                tooltipFormatter: (val, opts) => opts.seriesIndex === 0 ? formatPercent(val) : formatMegabytes(val),
                annotations: buildAnnotations(this.calculatePercentiles(points.map(p => p.memPct)), (v) => formatPercent(v))
            },
            {
                key: 'disk',
                el: 'perfDiskEl',
            ...
            },
            {
                ...
                tooltipFormatter: (val, opts) => {
                    ...
                    if (opts.seriesIndex === 0) {
                        return `${formatMegabytes(val)} total • ${formatMegabytes(point.netUploadMb)} up • ${formatMegabytes(point.netDownloadMb)} down`;
                    }
